...
def experiment_lobster(models_todo, dataset, now=None, servers=None, is_debug=False):
    ...
    for mod, plan in lunches_server:
        ...
        for see in seeds:
            ...
            for kp in horizons_wf:
                for km in horizons_wb:

                    print("Running FI experiment on {}, with K={}".format(mod, km, kp))
...
models_todo = {
    cst.Servers.ALIEN1: [(cst.Models.BINCTABL, {'k+': [cst.WinSize.SEC100], 'k-': [cst.WinSize.SEC100], 'seed': [500]})],
}


now = "LOBSTER-30-03-23"
