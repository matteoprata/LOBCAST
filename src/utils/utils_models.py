...
def get_tuned_parameters(config: Configuration, params):
    values = [config.TUNED.__getattribute__(p) for p in params]
    return values
...
def pick_model(config: Configuration, data_module):
    ...
    args = get_class_arguments(config.PREDICTION_MODEL.model)[2:]
    args_values = get_tuned_parameters(config, args)
    neural_architecture = config.PREDICTION_MODEL.model(num_features, num_classes, *args_values)
...
def pick_model_old(config: Configuration, data_module):
